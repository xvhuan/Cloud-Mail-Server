...
function parseAsUtc(time) {
    if (typeof time !== 'string') {
        return dayjs.utc(time);
    }

    const raw = time.trim();
    if (!raw) return dayjs.utc(raw);
    ...
    return dayjs.utc(`${normalized}Z`);
}

export function fromNow(date) {
    const d = parseAsUtc(date).tz(timeZone);
    const now = dayjs();
...
export function formatDetailDate(time) {
    const d = parseAsUtc(time).tz(timeZone);
    const now = dayjs();
...
export function tzDayjs(time) {
    return parseAsUtc(time).tz(timeZone)
}
